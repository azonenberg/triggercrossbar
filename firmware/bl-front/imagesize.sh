...
LINE=`ls -lh bootloader.bin | cut -d ' ' -f 5`
echo "Flash memory usage: $LINE";

STACKSTART=$(arm-none-eabi-objdump -t bootloader.elf  | grep __end | cut -d ' ' -f 1);
STACKEND=$(arm-none-eabi-objdump -t bootloader.elf  | grep __stack | cut -d ' ' -f 1);
...
DATASTART=$(arm-none-eabi-objdump -t bootloader.elf  | grep __data_start | cut -d ' ' -f 1);
DATAEND=$(arm-none-eabi-objdump -t bootloader.elf  | grep __data_end | cut -d ' ' -f 1);
...
BSSSTART=$(arm-none-eabi-objdump -t bootloader.elf  | grep __bss_start | cut -d ' ' -f 1);
BSSEND=$(arm-none-eabi-objdump -t bootloader.elf  | grep __bss_end | cut -d ' ' -f 1);
